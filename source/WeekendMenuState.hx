...
class WeekendMenuState extends MusicBeatState
{
	...
	public static var weekendData:Array<Dynamic> = [
		...
		[
			'Interrogation', 'Tha-Biscoot', 'Among-Us-Happy-Meal', 'Chuckie', '4.3', 'Scribble-Street', 'Scary-Junk', 'Thanos-Rumble', 'Normal-Ghost',
			'Plush-Factory', 'Spoar-Travel'
		],
	...
	override function create()
	{
		...
		if (FlxG.save.data.skipFlash == null)
			FlxG.save.data.skipFlash = false;
		if (FlxG.save.data.skipCopy == null)
			FlxG.save.data.skipCopy = false;
	...
	override function update(elapsed:Float)
	{
		...
		if (!movedBack && !paused && canSelect)
		{
			...
			if (controls.ACCEPT)
			{
				...
				{
					case 0:
						FlxG.save.data.skipFlash ? selectWeekend() : flashWarning('flashy');
					case 1 | 2:
						FlxG.save.data.skipCopy ? selectWeekend() : flashWarning('copyright');
					default:
						selectWeekend();
				}
			}
	...
	function flashWarning(img:String):Void
	{
		...
		warn.alpha = 0;
		add(warn);
		FlxTween.tween(warn, {alpha: 1}, 0.45, {
			onComplete: function(twn:FlxTween)
			{
				new FlxTimer().start(0, function(tmr:FlxTimer)
				{
					...
					{
						img == 'flash' ? FlxG.save.data.skipFlash = true : FlxG.save.data.skipCopy = true;
						warn.alpha = 0;
						selectWeekend();
					}
